...
name: use-modern-go
description: >
  WHEN: Any Go code work. Detects Go version from go.mod.
  WHEN NOT: Non-Go projects, Go versions below 1.21.
---
...
> **Priority:** Modern Go features override `uber-go-style` when newer syntax is available. For example, use `wg.Go()` (Go 1.25+) instead of the `sync.WaitGroup` pattern in the Uber guide.
